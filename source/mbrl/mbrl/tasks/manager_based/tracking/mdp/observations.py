...
def bad_joint_tracking_obs(env: ManagerBasedEnv, command_name: str, threshold: float) -> torch.Tensor:
    """Binary joint-tracking failure signal with explicit feature dimension for ObservationManager."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    return (torch.norm(command.joint_pos - command.robot_joint_pos, dim=-1) > threshold).unsqueeze(-1)
